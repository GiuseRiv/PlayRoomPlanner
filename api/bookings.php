...
function require_responsabile(PDO $pdo, int $uid): void {
  $st = $pdo->prepare("SELECT 1 FROM Settore WHERE id_responsabile=? LIMIT 1");
  $st->execute([$uid]);
  if (!$st->fetchColumn()) {
    err('Operazione consentita solo ai responsabili di settore', 403);
  }
}
...
function validate_slot(int $ora, int $dur): void {
  ...
  // Assunzione: l’evento deve finire entro le 23 (coerente col testo “ore comprese dalle 9 alle 23”)
...
try {
  if ($method === 'GET') {
    if ($id > 0) {
      ...
      // opzionale: filtra per organizzatore corrente
      $st = $pdo->prepare("
        SELECT p.*, s.nome AS nome_sala
        FROM Prenotazione p
        JOIN Sala s ON s.id_sala = p.id_sala
 ...
 if ($method === 'POST') {
  ...
  if ($method === 'PUT') {
    ...
    // carico prenotazione e controllo permessi (solo organizzatore)
    ...
    // vincolo sovrapposizioni stessa sala (escludendo se stessa)
